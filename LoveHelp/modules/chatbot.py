...
from LoveHelp import BOT_USERNAME as bu
from LoveHelp import BOT_ID, pbot, arq
from LoveHelp.ex_plugins.chatbot import add_chat, get_session, remove_chat
from LoveHelp.utils.pluginhelper import admins_only, edit_or_reply
...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Love AI Already Activated In This Chat")
            return
        await lel.edit(f"Love AI Actived by {message.from_user.mention()} for users in {message.chat.title}")

    elif status == "OFF" or status == "off" or status == "Off":
        lel = await edit_or_reply(message, "`Processing...`")
        Escobar = remove_chat(int(message.chat.id))
        if not Escobar:
            await lel.edit("Love AI Was Not Activated In This Chat")
            return
        await lel.edit(f"Love AI Deactivated by {message.from_user.mention()} for users in {message.chat.title}")
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        test = test.replace("@Lovesupport", "Lovebot.ai")
        test = test.replace("I live in @Lovesupport.", "I live in San Francisco, California.")
        test = test.replace("I was created by @excrybaby", "I made myself")
        test = test.replace(
            "Hello there I am Love...nice to meet u",
        ...
        response = response.replace("Aco", "Love")
        response = response.replace("aco", "Love")
        response = response.replace("Luna", "Love")
        response = response.replace("luna", "Love")
        ...
        response = response.replace("Lovebot.ai", "@emikksupport")
        response = response.replace("I live in San Francisco, California.", "I live in @Lovesupport.")
        ...
        response = response.replace("I'm married to my job.", "I'm married with @excrybaby")
        response = response.replace("I'm single.", "My husband is @excrybaby")
        response = response.replace("Lovebot.ai", "@Lovesupport")
...
@pbot.on_message(filters.regex("Love|Love|robot|Love|sena") & ~filters.bot & ~filters.via_bot  & ~filters.forwarded & ~filters.reply & ~filters.channel)
async def inuka(client, message):
    msg = message.text
    if msg.startswith("/") or msg.startswith("@"):
        message.continue_propagation()
...
__help__ = """
❂ Love AI is the only ai system which can detect & reply upto 200 language's
