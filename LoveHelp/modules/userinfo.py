...
from LoveHelp.__main__ import STATS, TOLove, USER_INFO
from LoveHelp.modules.sql import SESSION
import LoveHelp.modules.sql.userinfo_sql as sql
from LoveHelp.modules.disable import DisableAbleCommandHandler
from LoveHelp.modules.sql.global_bans_sql import is_user_gbanned
from LoveHelp.modules.sql.afk_sql import is_afk, set_afk
from LoveHelp.modules.sql.users_sql import get_user_num_chats
from LoveHelp.modules.helper_funcs.chat_status import sudo_plus
from LoveHelp.modules.helper_funcs.extraction import extract_user
from LoveHelp import telethn
    ...
    try:
        ...
        if user_member.status == "administrator":
            result = requests.post(
                f"https://api.telegram.org/bot{TOLove}/getChatMember?chat_id={chat.id}&user_id={user.id}",
            )
                    ...
                    [
                        [
                            InlineKeyboardButton(
                                "Health", url="https://t.me/LovenedyProject/44"),
                            InlineKeyboardButton(
                                "Disaster", url="https://t.me/LovenedyProject/43")
                        ],
                    ]
                ),
                parse_mode=ParseMode.HTML,
...
def stats(update: Update, context: CallbackContext):
    stats = "<b>╔═━「 Current Love Statistics 」</b>\n" + "\n".join([mod.__stats__() for mod in STATS])
    result = re.sub(r"(\d+)", r"<code>\1</code>", stats)
    result += "\n<b>╘═━「 Powered By Lovenedy-ex 」</b>"
    update.effective_message.reply_text(
        result,
        parse_mode=ParseMode.HTML, 
        disable_web_page_preview=True
...
*What is that health thingy?*
 Come and see [HP System explained](https://t.me/LovenedyProject/44)
"""
